...
function gf_trace_file(string $file, int $line): void
{
    ...
    <?= substr($file, strlen(gf_base_path()) + 1) ?><small class="text-black-50"><?= $line ? ':' . $line : '' ?></small>
    </div>
    <?php
}
            ...
            <?php foreach ($errors as $i => $err): ?>
                <div class="mb-4 tab-pane <?php if (!$i): ?>active shown<?php endif ?>" role="tabpanel" id="error-pane-<?= $i++ ?>"  aria-labelledby="error-tab-<?= $i ?>">
                    ...
                    <?php foreach ($err->getTrace() as $t): ?>
                        <?php if (!str_starts_with($t['file'] ?? '', gf_base_path() . '/vendor/')): ?>
                            <div class="px-3">
                                <?= $t['class'] ?? '' ?><?= $t['type'] ?? '' ?><span class="fw-bold"><?= $t['function'] ?></span>
                                <?php if (!isset($t['file'])): ?><div><small class="text-black-50">internal call</small></div><?php endif ?>
                            </div>
                        <?php endif ?>

                        <?php gf_trace_file($t['file'] ?? '', $t['line'] ?? 0) ?>
                    <?php endforeach ?>
